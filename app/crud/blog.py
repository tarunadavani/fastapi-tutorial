...
def destroy(id: int, db: Session, user: models.User):
    blog = db.query(models.Blog).filter(models.Blog.id == id, models.Blog.user_id == user.id)
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'Blog with the {id} is not available for user - {user.email}')
    blog.delete(synchronize_session=False)
    db.commit()
    return {'detail': f'Blog with {id} deleted successfully from db'}


def update(id: int, request: schemas.Blog, db: Session, user: models.User):
    blog = db.query(models.Blog).filter(models.Blog.id == id, models.Blog.user_id == user.id)
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f'Blog with the {id} is not available for user - {user.email}')
